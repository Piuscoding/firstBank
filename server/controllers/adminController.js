...
module.exports.allupgradesPage = async(req, res)=>{
  ...
  try {
    const payment = await paymentMoney.aggregate([ { $sort: { createdAt: -1 } } ])
      .skip(perPage * page - perPage)
      .limit(perPage)
      .exec(); 
    // const count = await Widthdraw.count();

    res.render('Allpayment',{payment});
...
module.exports.viewUprgadesPage = async(req, res)=>{
  ...
  try {
    const payment = await paymentMoney.findOne({ _id: req.params.id })

 res.render('viewpayment',{ payment})
...
module.exports.editUpgrade_post  = async(req, res)=>{
  try {
    await paymentMoney.findByIdAndUpdate(req.params.id,{
      amount: req.body.amount,
    ...
    await res.redirect(`/editpayment/${req.params.id}`);
...
module.exports.deleteUpgrade = async(req, res)=>{
  try {
    await paymentMoney.deleteOne({ _id: req.params.id });
    res.redirect("/adminRoute")
...
module.exports.allLoanPage = async(req, res)=>{
  ...
  try {
    const loans = await Loan.aggregate([ { $sort: { createdAt: -1 } } ])
      .skip(perPage * page - perPage)
      .limit(perPage)
      .exec(); 
    // const count = await Widthdraw.count();

    res.render('allLoans',{loans});
...
module.exports.viewLoansPage = async(req, res)=>{

  try {
    const loans = await Loan.findOne({ _id: req.params.id })

res.render('viewLoans',{ loans})
...
module.exports.editLoansPage= async(req, res)=>{
  try {
    const loans = await Loan.findOne({ _id: req.params.id })

   res.render('editLoans',{loans})
      } catch (error) {
        console.log(error);
      }
}

module.exports.editLoans_post = async(req, res)=>{
  try {
    ...
    res.location(`/editLoans/${req.params.id}`)
    // res.redirect('back');
    // await res.redirect(`/editLoans/${req.params.id}`);
...
module.exports.deleteLoans  = async(req, res)=>{
  try {
...
// ************************************FIXED DEPOSIT **************************//
module.exports.allFixedPage = async(req, res)=>{
  let perPage = 100;
  let page = req.query.page || 1;

  try {
    const fixed = await FixedDeposit.aggregate([ { $sort: { createdAt: -1 } } ])
      .skip(perPage * page - perPage)
      .limit(perPage)
      .exec(); 
    // const count = await Widthdraw.count();

    res.render('allFixed',{ fixed});
...
module.exports.viewFixedPage = async(req, res)=>{

  try {
    const fixed = await FixedDeposit.findOne({ _id: req.params.id })

      res.render('viewFixed',{fixed})
...
module.exports.editFixedPage = async(req, res)=>{
  try {
    const fixed = await FixedDeposit.findOne({ _id: req.params.id })

res.render('editFixed',{ fixed})
      } catch (error) {
        console.log(error);
      }
}

module.exports.editFixed_post  = async(req, res)=>{
  try {
    await FixedDeposit.findByIdAndUpdate(req.params.id,{
    ...
    await res.redirect(`/editFixed/${req.params.id}`);
    // res.redirect('back');
    console.log('redirected');
  } catch (error) {
    console.log(error);
...
module.exports.deleteFixed = async(req, res)=>{
  try {
